 ...
 * Reads up to 3 files, following one level of sitemap indexes. Failures are silent: no sitemap is normal.
 */
export async function loadSitemapLinks(
  ...
  while (queue.length > 0 && filesRead < MAX_SITEMAP_FILES && urls.length < MAX_URLS) {
    ...
    const res = await fetcher(next, { allowedTypes: XML_TYPES, maxBytes: 2_000_000 });
    if (!res.ok) continue;
